...
logging.info("Starting executor at %s", datetime.datetime.now())
...
class Executor(BaseHTTPRequestHandler):
    def do_POST(self):
        ...
        logging.info("HandlerDir: %s", handler_dir)
        ...
        logging.info("Module: %s, Function: %s", module, func_name)
        func_name = func_name[1:] # strip initial dot
        loaded_mod = None

        return_output = bool(request["ReturnOutput"])
        logging.info("ReturnOutput: %s", return_output)
        response = {}
